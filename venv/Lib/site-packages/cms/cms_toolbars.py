...
class BasicToolbar(CMSToolbar):
    ...
    def add_admin_menu(self):
        if not self._admin_menu:
            ...
            if len(sites_queryset) > 1:
                ...
                sites_menu.add_sideframe_item(_('Administrar sitios'), url=admin_reverse('sites_site_changelist'))
            ...
            # admin
            self._admin_menu.add_sideframe_item(_('Administración'), url=admin_reverse('index'))
            self._admin_menu.add_break(ADMINISTRATION_BREAK)

            # cms users settings
            self._admin_menu.add_sideframe_item(_('Configuración de usuario'), url=admin_reverse('cms_usersettings_change'))
            ...
            # Disable toolbar
            self._admin_menu.add_link_item(
                _('Deshabilitar barra de navegación'), url='?%s' % get_cms_setting('CMS_TOOLBAR_URL__DISABLE')
            )
    ...
    def add_users_button(self, parent):
        ...
        if User in admin.site._registry:
            ...
            if self.request.user.has_perm('%s.%s' % (opts.app_label, get_permission_codename('change', opts))):
                ...
                parent.add_sideframe_item(_('Usuarios'), url=user_changelist_url)
    ...
    def add_logout_button(self, parent):
        ...
        logout_menu_text = _('Cerrar sesión de %s') % user_name if user_name else _('Salir')
        ...
        if settings.USE_I18N and not self._language_menu:
            ...
            if len(languages) > 1:
                ...
                self._language_menu = self.toolbar.get_or_create_menu(
                    LANGUAGE_MENU_IDENTIFIER, _('Lenguaje'), position=-1
                )
...
class PageToolbar(CMSToolbar):
        ...
        if can_change:
            ...
            if add:
                add_plugins_menu = language_menu.get_or_create_menu(
                    '{0}-add'.format(LANGUAGE_MENU_IDENTIFIER), _('Añadir traducción')
                )
            ...
            if remove:
                remove_plugins_menu = language_menu.get_or_create_menu(
                    '{0}-del'.format(LANGUAGE_MENU_IDENTIFIER), _('Borrar traducción')
                )
    ...
    def change_admin_menu(self):
        ...
        if not self._changed_admin_menu and can_change_page:
            ...
            admin_menu.add_sideframe_item(_('Páginas'), url=url, position=0)
    ...
    def add_page_menu(self):
        if self.page and self.page_content:
            ...
            current_page_menu = self.toolbar.get_or_create_menu(
                PAGE_MENU_IDENTIFIER, _('Página'), position=1, disabled=self.in_apphook() and not self.in_apphook_root())
            ...
            add_page_menu_modal_items = (
                (_('Nueva página'), new_page_params, can_add_sibling_page),
                (_('Nueva sub página'), new_sub_page_params, can_add_sub_page),
            )
            ...
            current_page_menu.add_modal_item(_('Configuración de la página'), url=page_settings_url, disabled=settings_disabled,
                                             on_close=refresh)
            ...
            current_page_menu.add_modal_item(_('Configuración avanzada'), url=advanced_url, disabled=advanced_disabled)
            ...
            nav_title = _('Esconder en navegación') if in_navigation else _('Desplegar en navegación')
            ...
            current_page_menu.add_modal_item(_('Eliminar página'), url=delete_url, on_close=on_delete_redirect_url,
