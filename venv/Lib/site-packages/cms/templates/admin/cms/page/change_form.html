...
{% block title %}{% if add %}{% trans 'Add a page' %}{% else %}{% trans "Editar una página" %}{% endif %}{% endblock %}

{% block breadcrumbs %}
<div class="breadcrumbs">
    <a href="{% url 'admin:index' %}">{% trans 'Página principal' %}</a>
    ...
    &rsaquo; {% if add %}{% trans 'Agregar' %} {{ opts.verbose_name }}{% else %}{{ original|truncatewords:"18" }}{% endif %}
</div>
{% endblock %}
...
{% block content_title %}
    ...
    {% if not popup and not add %}
        <ul class="object-tools hide-in-modal">
            <li>
                <a href="{{ admin_url }}" target="_parent">{% trans "Ver en el sitio" %}</a>
            </li>
        </ul>
    {% endif %}
{% endblock %}
...
{% if errors %}
<p class="errornote">
{% blocktrans count errors|length as counter %}Por favor corrige el siguiente error.{% plural %}Por favor corrige los siguientes errores.{% endblocktrans %}
</p>
