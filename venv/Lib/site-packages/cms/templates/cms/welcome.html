    ...
    <div class="cms-reset cms-welcome">
        ...
        <section class="cms-welcome-section">
            <h2>
                {% if user.is_authenticated %}
                    {% trans "Agrega tu primera página" %}
                {% else %}
                    {% trans "Por favor inicia sesión" %}
                {% endif %}
            </h2>
            <p>
                {% blocktrans %}
                   Bienvenido a DjangoCMS, una herramienta para la creación de contenido web de forma dinámica. 
                   Estás usando la versión <strong>{{ cms_version }}</strong>. Para acceder registrate y espera que un 
                   administrador apruebe tu solicitud para que pronto puedas empezar a trabajar dentro de la herramienta.

                   Dentro de ella encontrarás varios elementos que te ayudarán a que tu instancia en uso sea lo más cómodo posible.
                {% endblocktrans %}
            </p>
            <p>
                ...
                {% if user.is_authenticated %}
                    {% blocktrans %}
                        <a href="{{ admin_add_page }}" class="js-welcome-add">Agrega tu primera página</a> para continuar.
                    {% endblocktrans %}
                {% else %}
                    {% blocktrans %}
                        Por favor 
                        <a href="{{ admin_add_page }}" class="js-welcome-add">crea una página</a> manualmente.
                    {% endblocktrans %}
                {% endif %}
            </p>
        </section>
        ...
        <footer class="cms-welcome-footer">
            ...
            <p class="cms-welcome-links">
                ...
                <a href="//django-cms.org/en/support" target="_blank">{% trans "Soporte" %}</a>
                <a href="//docs.django-cms.org/" target="_blank">{% trans "Documentación" %}</a>
            </p>
            {% comment %} {% blocktrans %}
            ...
            CMS.$(function () {
                ...
                btn.on('click', function (e) {
                    ...
                    modal.open({
                        ...
                        title: '{% trans "Bienvenido a Django CMS" %}'
                    });
                });
