...
<form id="" action="{% url 'save' 1 %}" method="POST">
    ...
    {% if grupos %}
         <label><b>ASIGNACIONES:<b></label>
          ...
          {% comment %} {% for grupo in grupos %}
            <tr>
                  <label><b>ASIGNACIÓN {{forloop.counter}}: <b></label>
                  </tr>
                  <tr>
  ...
  <div class="submit-row">
      ...
      {% endif %}
      <div class="fila">
        <a  class="cms-btn-action" href="{% url 'actividades' 1 %}">{% translate 'Back' %}</a>
      </div>
  </div> 
