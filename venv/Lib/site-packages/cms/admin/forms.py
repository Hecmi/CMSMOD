...
class BasePageContentForm(forms.ModelForm):
    ...
    title = forms.CharField(
        ...
        help_text=_('El título identificador')
    )
    slug = forms.SlugField(
        label=_("Etiqueta"),
        max_length=255,
        help_text=_('Parte del título que se considerará en la URL')
    )
    menu_title = forms.CharField(
        label=_("Título del menú"),
        widget=forms.TextInput(),
        help_text=_('Sobreescribe lo que se mostrará como título'),
        required=False
    )
    page_title = forms.CharField(
        label=_("Título de la página"),
        widget=forms.TextInput(),
        required=False,
        help_text=_('Sobreescribe lo que se mostrará como título en el navegador'),
    )
    meta_description = forms.CharField(
        label=_('Description de etiqueta meta'),
        max_length=320,
        required=False,
        widget=forms.Textarea(
            attrs={'maxlength': '320', 'rows': '4'}
        ),
        help_text=_('Escribe una descripción que ayude a los navegadores a encontrar tu página.'),
    )
...
class ChangePageForm(BasePageContentForm):
    overwrite_url = forms.CharField(
        label=_('Sobreescribe la URL'),
        max_length=255,
        required=False,
        help_text=_('Déjalo vacío si quieres usar la URL estándar'),
    )
    soft_root = forms.BooleanField(
        label=_('Ruta suave'),
        required=False,
        help_text=_("Las páginas pasadas no se mostrarán en la navegación"),
    )
    redirect = PageSmartLinkField(
        label=_('Redireccionar'),
        required=False,
        help_text=_('Redicciona a esta URL'),
        placeholder_text=_('Empieza a escribir...'),
    ...
    limit_visibility_in_menu = forms.TypedChoiceField(
        ...
        label=_("Visibilidad de menú"),
        help_text=_("Límite cuando esta página sea visible"),
    ...
    fieldsets = (
        (None, {
            'fields': ('title', 'slug', 'template', 'page_title', 'meta_description'),
        }),
        (_('URL options'), {
    ...
    def clean(self):
        ...
        if path_override:
            ...
            path = u'%s/%s' % (base_path, slug) if base_path else None
        else:
            path = slug
...
class MovePageForm(PageTreeForm):
    ...
    def clean(self):
        ...
        if self.page.is_home and cleaned_data.get('target'):
            self.add_error('target', force_str(_('No puedes mover esta página a otra')))
        return cleaned_data
...
class GenericCmsPermissionForm(forms.ModelForm):
    ...
    can_add_page = forms.BooleanField(label=_('Agregar'), required=False, initial=True)
    can_change_page = forms.BooleanField(label=_('Editar'), required=False, initial=True)
    can_delete_page = forms.BooleanField(label=_('Eliminar'), required=False)

    # pageuser is for pageuser & group - they are combined together,
    # and read out from PageUser model
    can_add_pageuser = forms.BooleanField(label=_('Agregar'), required=False)
    can_change_pageuser = forms.BooleanField(label=_('Editar'), required=False)
    can_delete_pageuser = forms.BooleanField(label=_('Eliminar'), required=False)

    can_add_pagepermission = forms.BooleanField(label=_('Agregar'), required=False)
    can_change_pagepermission = forms.BooleanField(label=_('Editar'), required=False)
    can_delete_pagepermission = forms.BooleanField(label=_('Eliminar'), required=False)
