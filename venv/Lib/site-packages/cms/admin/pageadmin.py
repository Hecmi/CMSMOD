...
class PageAdmin(admin.ModelAdmin):
    ...
    def set_home(self, request, object_id):
        ...
        if not page.is_potential_home():
            return HttpResponseBadRequest(force_str(_("Esta página no puede ser la principal.")))
    ...
    def delete_view(self, request, object_id, extra_context=None):
        ...
        if request.POST and not protected:  # The user has confirmed the deletion.
            ...
            self.message_user(
                request,
                _('El %(name)s "%(obj)s" fueeliminado correctamente.') % {
        ...
        if perms_needed or protected:
            title = _("No puedes borrar %(name)s") % {"name": object_name}
        else:
    ...
    def add_view(self, request, form_url='', extra_context=None):
        ...
        if 'duplicate' in request.path_info:
            extra_context.update({
                'title': _("Copiar página"),
            })
        elif 'parent_node' in request.GET:
            extra_context.update({
                'title': _("Nueva sub página"),
            })
        else:
            extra_context.update({
                'title': _("Nueva página"),
            })
    ...
    def change_innavigation(self, request, object_id):
        ...
        if not self.has_change_permission(request, obj=page_content):
            if self.has_change_permission(request):
                ...
                message = _("No tienes permisos para cambiar el estado de una página")
            else:
                # Only this page? Can be permissions or versioning, or ...
                message = _("No puedes cambiar el estado de la página")
