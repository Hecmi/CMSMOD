...
class CreateCMSPageForm(AddPageForm):
    ...
    content = forms.CharField(
        label=_('Contenido'), widget=text_widget, required=False,
        help_text=_("Opcional. Si lo defines, se establecerá en un elemento de tipo texto.")
    )
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['title'].help_text = _("Define un título para la nueva página.")
        self.fields['slug'].required = False
        self.fields['slug'].help_text = _("Deja vacío para que el slug se defina automáticamente o sobreescribelo si deseas.")
    ...
    def clean(self):
        ...
        if not slug:
            ...
            forms.ValidationError({
                "slug": [_("No se pudo crear el slug, por favor escribe uno.")],
            })
        ...
        if not data['slug']:
            raise forms.ValidationError(_("Por favor provee un slug válido."))
        return data
    ...
    def clean_parent_node(self):
        ...
        if not has_perm:
            message = gettext('No tienes permisos para añadir una página.')
