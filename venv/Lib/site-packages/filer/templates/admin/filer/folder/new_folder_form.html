{% extends "admin/change_form.html" %}
{% load i18n filer_admin_tags %}

{% block title %}{% translate "Agregar nueva" %} {% translate "carpeta" %} | {{ site_title|default:_('Sitio de administración') }}{% endblock %}

{% block content %}
    <h1>{% translate "agregar nueva" %} {% translate "carpeta" %}</h1>
    <div id="content-main">
        <form id="folder_form" method="post" action="" enctype="multipart/form-data">
            {% csrf_token %}
            <div>
                {% filer_admin_context_hidden_formfields %}
                {% if new_folder_form.errors %}
                    <p class="errornote">
                        {% blocktrans count new_folder_form.errors|length as counter %}Por favor corrige el siguiente error.{% plural %}Por favor corrige los siguientes errores.{% endblocktrans %}
                    </p>
                    {{ new_folder_form.non_field_errors }}
                {% endif %}
                <fieldset class="module aligned">
                    <div class="form-row {% if new_folder_form.errors.name %}errors {% endif %}name">
                        {{ new_folder_form.errors.name.as_ul }}
                        <div>
                        {% for field in new_folder_form %}
                            <label class="required" for="id_{{ field.name }}">{{ field.label }}:</label>{{ field }}
                        {% endfor %}
                        </div>
                    </div>
                </fieldset>
                <div style="overflow: auto;" class="submit-row">
                    <input type="submit" name="_save" class="default" value="{% trans 'Save' %}">
                </div>
                <script type="text/javascript">
                    document.getElementById('id_name').focus();
                </script>
            </div>
        </form>
    </div>
{% endblock %}
