...
class GridContainerForm(
    ...
    container_type = forms.ChoiceField(
        label=_("Tipo de contenedor"),
        choices=GRID_CONTAINER_CHOICES,
        initial=first_choice(GRID_CONTAINER_CHOICES),
        help_text=_(
            "Define la configuración del tipo de contenedor seleccionado, desde los márgenes hasta el espaciado y colores."
        ),
    )
...
class GridRowBaseForm(
    ...
    create = forms.IntegerField(
        label=_("Número de columnas"),
        help_text=_("Número de columnas que tendrá la matriz cuando sea creada."),
        required=False,
        min_value=0,
        max_value=GRID_SIZE,
    )
    vertical_alignment = forms.ChoiceField(
        label=_("Alineamiento vertical"),
    ...
    horizontal_alignment = forms.ChoiceField(
        label=_("Alineamineto horizontal"),
    ...
    gutters = forms.ChoiceField(
        ...
        required=False,
        help_text=_("Para remover los espacios entre las filas, defina el valor de cero."),
        ...
        for size in settings.DEVICE_SIZES:
            if f"{size}_col" in self.cleaned_data:
                ...
                raise ValidationError(
                    _(
                        'El tamaño no puede ser vacío, elija "auto", o un '
                        "número entre 1 y %(cols)d"
                    ),
