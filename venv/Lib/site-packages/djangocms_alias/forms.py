    ...
    def clean(self):
        ...
        if not plugin and not placeholder:
            raise forms.ValidationError(
                _('Un plugin or placeholder es requerido para crear un alias.')
            )

        if plugin and placeholder:
            raise forms.ValidationError(
                _(
                    'Un plugin or placeholder es requerido para crear un alias, '  # noqa: E501
                    'no ambos.'
                )
            )
...
class CreateAliasForm(BaseCreateAliasForm):
    ...
    replace = forms.BooleanField(
        label=_('Reemplazar el actual alias'),
        help_text=_('Reemplazar el actual alias'),
        required=False,
    )
...
class AliasGrouperAdminForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if AliasContent.admin_manager.filter(
            ...
            raise forms.ValidationError(
                _('Ya existe un alias con este nombre y categoría')
            )
