...
<body>
    <div id="cms-top" class="cms cms-reset" data-compare='{
        ...
        {% endif %}
        {% if v2 %}
            "v2_description": "{{ v2_description|default:v2.verbose_name }}",
        {% endif %}
        "v1_url": "{{ v1_preview_url }}",
        "v1_description": "{{ v1_description|default:v1.verbose_name }}"
    }'>
        <div class="cms-toolbar">
            <div class="cms-versioning-controls">
                {% if return_url %}
                    <a class="cms-btn" href="{{ return_url }}">{% translate "Regresar" %}</a> &nbsp;
                {% endif %}
                <span class="cms-versioning-title">
                {% blocktrans with left=v1_description|default:v1.verbose_name %}
                    Comparando {{ left }} con
                {% endblocktrans %}
                </span>
                <select class="js-cms-versioning-version cms-select">
                    <option disabled {% if not request.GET.compare_to %} selected{% endif %}>{% translate "Selecciona una versión para comparar" %}</option>
                    {% for version in version_list %}
                        {% if version.pk != v1.pk %}
                            <option value="{{ version.pk }}" {% if v2 and v2.pk == version.pk %} selected{% endif %}>{{ version.verbose_name }}</option>
                        {% endif %}
                    {% endfor %}
                </select>
