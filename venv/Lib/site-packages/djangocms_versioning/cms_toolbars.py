...
class VersioningToolbar(PlaceholderToolbar):
    ...
    def _add_edit_button(self, disabled=False):
        ...
        if version.check_edit_redirect.as_bool(self.request.user):
            ...
            item.add_button(
                _("Editar") if draft_exists else _("Nueva versión"),
                url=edit_url,
                disabled=disabled,
    ...
    def _add_versioning_menu(self):
        ...
        if self.request.user.has_perm(
            ...
            versioning_menu.add_sideframe_item(_("Gestionar versiones"), url=url)
            # Compare to source menu entry
            if version.source:
                name = _("Comparar con versión {source}").format(source=_(version.source.short_name()))
                proxy_model = self._get_proxy_model()
                url = reverse(
...
class VersioningPageToolbar(PageToolbar):
    ...
    def override_language_menu(self):
        ...
        language_menu = self.toolbar.get_menu(LANGUAGE_MENU_IDENTIFIER, _("Lenguaje"))
        ...
        if can_change:
            ...
            if add:
                ...
                add_plugins_menu = language_menu.get_or_create_menu(
                    f"{LANGUAGE_MENU_IDENTIFIER}-add", _("Añadir traducción")
                )
            ...
            if copy:
                copy_plugins_menu = language_menu.get_or_create_menu(
                    f"{LANGUAGE_MENU_IDENTIFIER}-copy", _("Copiar todos los plugins")
                )
                title = _("De %s")
                question = _("¿Estás seguro de querer copiar el plugin de %s?")
                item_added = False
                for code, name in copy:
                    ...
                    if not item_added:  # pragma: no cover
                        copy_plugins_menu.add_link_item(
                            _("No hay otro lenguaje disponible"),
                            url="#",
                            disabled=True,
                        )
